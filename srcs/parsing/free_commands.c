...
/*   Updated: 2023/04/10 01:00:15 by nicolas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "minishell.h"
...
void	free_lexer(t_lexer *lexer)
{
	...
	while (lexer)
	{
		...
		if (temp->exec)
			free(temp->exec);
		if (temp->options)
			free_str_arr(temp->options);
		if (temp->args)
			free_str_arr(temp->args);
		if (temp->redir_path[0])
			free(temp->redir_path[0]);
		if (temp->redir_path[1])
			free(temp->redir_path[1]);
		free(temp);
	}
}
